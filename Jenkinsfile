pipeline {
    ...
    environment {
        IMAGE_NAME = 'avs-rgr'   //Имя докер-контейнера, -образа.
        DOCKER_IMAGE = 'avs-rgr:latest'
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/GeorgiiIgnatenko/avs-rgr.git'  // Клонируем ветку с репозитория
            }
        }
